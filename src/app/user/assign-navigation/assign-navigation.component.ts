...
export class AssignNavigationComponent {
  ...
  enter_by:any=1 //after session it will
...
//session ended
  ...
  getuserRole: any;
    fetchUserrole() {
    this.allService.getRole().subscribe((data:any) => {
      this.getuserRole = data.data;  
    });
  }
 ...
 fetchnavPage() {
  ...
  // if (result) {
  //   this.allService. deleteAll_nav(selectedRole).subscribe((result:any)=>{  //for delete
  //     alert(result)
  
  //     this.allService.searchAssignNav(this.selectedRole).subscribe((getres:any)=>{ //for dynamic reload
